...
vec3 castRay(Ray ray, int depth = 0){
    ...
    //计算折射到这里的颜色
    vec3 refract_dir = normalize(refract(ray.dir,n,material.refractive_index));  //refract(dir, N, material.refractive_index).normalize();
    ...
    for(int i=0;i<lights.size();i++){
        ...
        vec3 shadow_orig = dot(l,n) < 0 ? hitPos - n*EPSILON : hitPos + n*EPSILON; // checking if the point lies in the shadow of the lights[i]
